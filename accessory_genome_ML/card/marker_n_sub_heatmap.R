...
# marker table ----
marker_tab <- read.csv("res_marker.csv",
                       #sep = ",",
                       encoding = "UTF-8",
                       header = FALSE,
                       # row.names = FALSE
...
# get needed columns ----
needed_cols <- c()
for (x in colnames(marker_tab)[2: 261]) {
...
#hm
ggsave("E:/adcadmic/Prof_guo/all_ec2/card/sub_heatmap.pdf", 
       hm,
       height = 16, 
       width = 9)
...
# marker_tab_wei['TEM-206']
# heatmap_df_CARD_redo[, "TEM-206"]
# 
# marker_tab_wei[1, ] %>% unname() %>% unlist() %>% unique() %>% sort()
...
ggsave("E:/adcadmic/Prof_guo/all_ec2/card/marker_heatmap.pdf", 
       hm_marker,
       height = 16, 
       width = 9)
